...
export default function RedactionList({
  ...
  const rows = useMemo(() => {
    ...
    return metadata.map((metadataRow) => {
      ...
      return {
        ...
        format_nom: redaction?.format_nom || "",
  ...
  const filtered = useMemo(() => {
    return rows.filter((item) => {
      ...
      if (formatFilter && String(item.format_id) !== String(formatFilter)) return false;
      if (traiteFilter !== "" && String(Boolean(item.traite)) !== traiteFilter) return false;
      return true;
    });
  }, [coupureFilter, echelleFilter, feuilleFilter, formatFilter, rows, traiteFilter]);

  const hasFilters = feuilleFilter || coupureFilter || echelleFilter || formatFilter || traiteFilter;
  ...
  return (
    <section className="card">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Rédactions cartographiques</h2>
          <p className="mt-1 text-sm text-slate-600">
            {filtered.length} / {rows.length} métadonnée(s)
          </p>
        </div>
        {hasFilters && (
          <button
      ...
      {filtered.length === 0 ? (
        <p className="mt-4 text-sm text-slate-600">Aucune rédaction cartographique.</p>
      ) : (
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((item) => {
                ...
                return (
                  <tr key={item.id} className={isEditing ? "bg-primary-50" : "hover:bg-slate-50"}>
                    ...
                    <td className="px-3 py-2 text-slate-700">{item.operateur_nom || "-"}</td>
                    <td className="px-3 py-2 text-slate-700">{item.date_debut || "-"}</td>
                    <td className="px-3 py-2 text-slate-700">{item.date_fin || "-"}</td>
                    <td className="px-3 py-2 text-slate-700">{item.logiciel_utilise || "-"}</td>
                    <td className="px-3 py-2 text-slate-700">{item.version_logiciel || "-"}</td>
                    <td className="px-3 py-2 text-slate-700">{item.format_nom || "-"}</td>
