...
export default function Redaction({ metadata = [], metadataForRedaction = [], redactions = [] }) {
  ...
  return (
    <>
      <Head title="Redaction cartographique" />

      <main className="page-shell">
        <div className="page-container max-w-5xl">
          <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Redaction cartographique</h1>
          ...
          {(flashSuccess || recentlySuccessful) && (
            <div className="mb-4 rounded border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
              {flashSuccess || "Redaction cartographique creee avec succes."}
            </div>
          )}
